...
introduce="거주지 : 8월 중 G밸리하우스디어반 입주 예정\nMBTI : ISTP\n취미 : 시체놀이\n꿈 : 40세 이전 은퇴 후 날먹"
    ...
    if req.type == "events_api":
        ...
        if req.payload["event"]["type"] == "message":
            ...
            if slack_subtype is None:
                ...
                print("나여기이이이이이이",channel_id)
                present_ts = ''
                thread_ts = req.payload["event"]['ts']
                if '장어진' in slack_text:
                    SLACK_CLIENT.web_client.chat_postMessage(channel=channel_id,text=introduce)
